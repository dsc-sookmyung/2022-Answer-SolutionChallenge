...
import { useToast, Button, HStack, Text, Divider, Modal, VStack, Box } from 'native-base';
...
export default function TranslateScreen({ navigation }: Navigation) {
  ...
  const handleKorean = (): void => {
	...
	const copyToClipboard = () => {
		if (showKorean) {
			if (results?.korean) {
				Clipboard.setString(results.korean);
			}
		}
		else {
		...
		toast.show({    // Design according to mui toast guidelines (https://material.io/components/snackbars#anatomy)
			...
			render: () => {
				return <Box bg="rgba(0,0,0,0.7)" p="4" rounded="xl" mx={2} shadow={2} alignItems="center">
						<Text color="white">{i18n.t('copiedToClipboard')}</Text>
					</Box>;
			}
		});
	};
      ...
      {imageUri ? (
              ...
              <Modal isOpen={openInitialEventForm} animationPreset="slide">
                <Modal.Content maxWidth="400">
                    ...
                    <VStack space={2}>
                      {results?.event_num ? (
                        <Text fontSize="lg" fontWeight={600} textAlign="center">
                          We found {results.event_num} events for you
                        </Text>
                      ) : (
                        <Text fontSize="lg" fontWeight={600} textAlign="center">
                  ...
                  <Modal.Footer alignSelf="center" bgColor="muted.50" p={2}>
                    <Button
                      variant="unstyled"
                      onPress={() => setOpenInitialEventForm(false)}
                    >
                      OK
                    </Button>
                  </Modal.Footer>
        ...
        loading ? (
          ...
          <>
            ...
            >
              <TouchableOpacity
                style={[styles.circleButton, styles.primaryBackground]}
                onPress={extractText}
              >
        ...
        <>
          ...
          >
            ...
            <TouchableOpacity
              style={[styles.circleButton, styles.whiteBackground]}
              onPress={takePicture}
            >
